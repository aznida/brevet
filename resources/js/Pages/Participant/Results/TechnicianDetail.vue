<template>
    <div>
        <Head>
            <title>Detail Teknisi - {{ technician.name }}</title>
        </Head>
        <LayoutParticipant>
                    ...
                    <div class="col-12 mb-4">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 text-end">
                                        <Link href="/participant/results" class="btn btn-sm btn-primary">
        ...
        <p class="mb-0">
            <i class="fa fa-info-circle me-2"></i>Nama : <b>{{ technician.name }}</b> <br>
            <span style="margin-left:25px">NIK : {{ technician.nik }}</span><br>
            <span style="margin-left:25px">Witel : {{ technician.witel }}</span><br>
            <!-- <span style="margin-left:25px">Treg : {{ technician?.area?.title }}</span><br> -->
            <span style="margin-left:25px">Level : 
                ...
                {{ getUserLevel.emoji }} {{ getUserLevel.level.toUpperCase() }}
                                    ...
                                    <div class="row mb-4" style="margin-left: 0px;padding-left: 0px;padding-right: 0px;margin-right: 0px; width:1360px">
                            ...
                            <table class="table table-bordered table-centered table-nowrap mb-0 rounded">
                                ...
                                <div class="mt-2 text-end">
                                    <small class="text-muted">
                                        <i><b>Nilai Teknisi:</b> </i> 
<span class="badge bg-warning">
    {{ Number(topTechniciansArea?.find(tech => 
        String(tech.participant_id) === String(technician.id))?.average_grade || 0).toFixed(2) 
    }}</span>   
                                        <i><b>     Posisi:</b> </i> 
                                        <span class="badge bg-success">#{{ 
                                            topTechniciansArea.findIndex(tech => 
                                                String(tech.participant_id) === String(technician.id)) + 1 
                                        }}</span>
                                    </small>
                                </div>
                        </div>
            ...
            <div class="col-12 col-md-6">
                <div class="card border-0 shadow">
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-centered table-nowrap mb-0 rounded">
                                <!-- Untuk tabel Top Teknisi Regional sudah ada width="1%", tidak perlu diubah -->
                                
                                <!-- Untuk tabel Top Teknisi Nasional, perlu ditambahkan width="10px" -->
                                <thead class="thead-dark">
                                    <tr>
                                        <th width="10px">NO.</th>
                                        <th>NAMA</th>
...
<span class="badge bg-warning">
    {{ Number(topTechniciansNational?.find(tech => 
        String(tech.participant_id) === String(technician.id))?.average_grade || 0).toFixed(2) 
                              ...
                              <div class="row" style="margin-left: 0px;padding-left: 0px;padding-right: 0px;margin-right: 0px; width:1360px">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-centered table-nowrap mb-0 rounded">
                                <thead class="thead-dark">
                                    <tr class="border-0">
                                        <th class="border-0" width="10px">No.</th>
...
<script>
    ...
    export default {
        ...
        //mounted
        mounted() {
            console.log('Technician Detail Component Mounted.');
        ...
        computed: {
            ...
            getUserLevel() {
                ...
                if (this.averageGrade > 90) {
                    level = { level: 'expert', emoji: '💎', averageGrade: this.averageGrade };
                } else if (this.averageGrade > 70 && this.averageGrade <= 90) {
                    level = { level: 'advanced', emoji: '🥇', averageGrade: this.averageGrade };
                } else if (this.averageGrade > 60 && this.averageGrade <= 70) {
                    level = { level: 'intermediate', emoji: '🥈', averageGrade: this.averageGrade };
                } else if (this.averageGrade > 30 && this.averageGrade <= 60) {
                    level = { level: 'basic', emoji: '🥉', averageGrade: this.averageGrade };
                }
                
                return level;
            },
            
            // Implementasi radarSeries untuk Score Map
            radarSeries() {
                ...
                const data = this.categories.map(category => {
                    return this.categoryScores[category.title] || 0;
                });
            ...
            // Implementasi radarChartOptions
            radarChartOptions() {
                return {
                    chart: {
                        type: 'radar',
            ...
            // Implementasi assessmentSeries untuk Hasil Assessment
            assessmentSeries() {
                if (!this.categoryScores || Object.keys(this.categoryScores).length === 0) {
                    return [];
                }
            ...
            // Implementasi assessmentChartOptions
